...
(function(root, factory) {
    ...
    return {
        ...
        generateSchema: function(schema) {
            ...
            //   alert(isAndroidChrome)
            // 如果是安卓chrome浏览器，则通过intent方式打开
            if (isAndroidChrome) {
                //alert(AppConfig.PROTOCAL)
                schemaStr = "intent://" + schemaStr + "#Intent;" +
                    "scheme=" + AppConfig.PROTOCAL + ";" +
                    "package=" + AppConfig.APK_INFO.PKG + ";" +
                    (AppConfig.APK_INFO.CATEGORY ? ("category=" + AppConfig.APK_INFO.CATEGORY + ";") : "") +
                    (AppConfig.APK_INFO.ACTION ? ("action=" + AppConfig.APK_INFO.ACTION + ";") : "") +
                    "S.browser_fallback_url=" + encodeURIComponent(AppConfig.FAILBACK.ANDROID) + ";" +
                    "end";
                // alert(schemaStr)
                // schemaStr = 'intent://jiuwo.xunlei.com/wap/room/play.html#Intent;scheme=jiuwo://invoke?roomid=12401&platform=1&http://flv2.jiuwo.xunlei.com/live/11472.flv;package=com.xunlei.xllive;S.browser_fallback_url=http://jiuwo.xunlei.com/act/app/wap;end'
